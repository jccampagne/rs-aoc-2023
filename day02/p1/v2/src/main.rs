...
impl Parser {
    ...
    fn parse_line(&self, line: &str) -> Option<GameId> {
        ...
        let mut exceeds_limits = 0;
        self.check_all_draws_for_limit(part_draws, &mut exceeds_limits);
    ...
    fn check_all_draws_for_limit(&self, part_draws: Option<&str>, exceeds_limits: &mut i32) {
        let Some(part_draws) = part_draws else {
            panic!("bad input");
        };
        let draws = part_draws.split(";");
        for draw in draws {
            self.check_one_draw_limit(draw, exceeds_limits);
        }
    }

    fn check_one_draw_limit(&self, draw: &str, exceeds_limits: &mut i32) {
