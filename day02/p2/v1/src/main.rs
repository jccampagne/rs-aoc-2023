...
struct Parser {
    counter: HashMap<String, i128>,
}

impl Parser {
    pub fn compute(&mut self, input: &str) -> Number {
        let result = input.lines().filter_map(|line| self.parse_line(line)).sum();
        return result;
    }

    fn parse_line(&mut self, line: &str) -> Option<GameId> {
        ...
        for draw in draws {
            ...
            for nc in number_color_list {
                ...
                // dbg!((number, color));
                match self.counter.get(color) {
                    None => _ = self.counter.insert(color.into(), number),
                    Some(previous_number) => {
                        if *previous_number < number {
                            _ = self.counter.insert(color.into(), number)
                        }
                    }
                }
            }
...
fn main() {
    ...
    let mut parser = Parser::from(LIMITS);
